  ...
  function expToLevel(table, total) {
    ...
    for(var i=1; i<totals.length; i++){
      if(totals[i] > total){
        ...
        var next = table[i];
        var parcent = Math.floor(1000 * exp / next)/10;
  ...
  function recalc() {
    ...
    for(var i=1; i<expCards.length; i++){
      ...
      if(amount){
        ...
        // console.log('gain Lv'+i+' amount:'+amount+' card '+JSON.stringify(card)+' => '+JSON.stringify(stats));
      }
    }
    if(errors.length){
    ...
    // $('#result_log').val('RESULT:'+JSON.stringify(stats)+' ERRORS:'+JSON.stringify(errors));
